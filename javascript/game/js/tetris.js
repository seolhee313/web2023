...
function checkGameOver() {
    ...
    const topLine = childNodes[0].childNodes[0].childNodes;
    const gameOver = Array.from(topLine).some((li) =>
        li.classList.contains("seized")
    );
    if (gameOver) {
...
function generateNewBlock() {
    ...
    movingItem.top = 0;
    movingItem.left = Math.floor(line_cols / 2) - 1;
    movingItem.direction = 0;
    tempMovingItem = { ...movingItem };
    renderBlocks();

    checkGameOver(); // 새로운 블록 생성 후 게임 오버 체크
}
